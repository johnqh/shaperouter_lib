# Improvement Plans for @sudobility/shaperouter_lib
...
- Testing requires mocking `@sudobility/shaperouter_client` hooks and verifying that store state is correctly updated when client data changes.
...
- Like `shaperouter_client`, this project lacks a `verify` script. The CLAUDE.md instructs running `bun run typecheck && bun run lint && bun run test:run && bun run build` manually.
- Adding `"verify": "bun run typecheck && bun run lint && bun run test:run && bun run build"` to `package.json` would align with the `shaperouter_types` convention and reduce friction.
