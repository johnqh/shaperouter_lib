...
Business logic library for ShapeRouter with Zustand stores, manager hooks, and endpoint templates (v0.0.84).

**npm**: `@sudobility/shaperouter_lib`
...
```typescript
import { useKeysStore } from '@sudobility/shaperouter_lib';
...
- **Budget store** (`useBudgetTracker`): Uses localStorage internally via Zustand `persist` middleware. Key: `'shaperouter-provider-models'`.
...
Manager hooks wrap `shaperouter_client` hooks with Zustand store caching. They follow a consistent pattern:
...
} from '@sudobility/shaperouter_lib';
```
...
3. Wrap the corresponding client hook from `@sudobility/shaperouter_client`
...
- `@sudobility/shaperouter_client` - API hooks
...
shaperouter_app (frontend)
    └── shaperouter_lib (this package - stores, managers, templates)
        └── shaperouter_client (API hooks)
            └── shaperouter_types (types)
```
...
| ------------------- | ------------ |
| `shaperouter_app` | Direct dependency - uses stores, manager hooks, and templates |

After making changes:

1. `bun run verify`
2. `npm publish`
3. In `shaperouter_app`: `bun update @sudobility/shaperouter_lib` -> rebuild
...
# In shaperouter_app:
bun link @sudobility/shaperouter_lib

# If also changing shaperouter_client, link it first:
cd ../shaperouter_client && bun link
cd ../shaperouter_lib && bun link @sudobility/shaperouter_client

# Rebuild after changes:
bun run build

# When done, unlink:
bun unlink @sudobility/shaperouter_lib && bun install
```
...
- **Peer dependency on `shaperouter_client`** -- this package wraps client hooks. If you update `shaperouter_client`, verify compatibility here.
